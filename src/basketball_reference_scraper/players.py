...
def get_stats(_name, stat_type='PER_GAME', playoffs=False, career=False, ask_matches = True):
    ...
    if r.status_code==200:
        ...
        return df
def get_salaries(_name, ask_matches=True):
    name = lookup(_name, ask_matches)
    suffix = get_player_suffix(name).replace('/', '%2F')
    selector = 'all_salaries'
    r = get(f'https://widgets.sports-reference.com/wg.fcgi?css=1&site=bbr&url={suffix}&div=div_{selector}')
    if r.status_code==200:
        soup = BeautifulSoup(r.content, 'html.parser')
        table = soup.find('table')
        df = pd.read_html(str(table))[0]
        df.rename(columns={'Season' : 'SEASON', 'Salary' : 'SALARY'}, inplace=True)
        df = df.drop(['Team', 'Lg'], axis=1)
        return df

def get_2021_salaries():
    selector = 'all_salaries'
    r = get(f'https://widgets.sports-reference.com/wg.fcgi?css=1&site=bbr&url=%2Fcontracts%2Fplayers.html&div=div_player-contracts')
    if r.status_code==200:
        soup = BeautifulSoup(r.content, 'html.parser')
        table = soup.find('table')
        df = pd.read_html(str(table))[0]
